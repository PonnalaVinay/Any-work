...
function createTable(rows) {
  ...
  const table = `<table border="2" style="background:linear-gradient(rgb(176, 130, 188),hsl(130,80%,45%,0.4),rgb(131, 209, 183));margin-left:550px">
                  <tr>
                    <th>Natural Index</th>
                    <th>Name</th>
                    <th>Number</th>
  ...
  const pageWithRedBackground = `<body style="background:linear-gradient(hsl(230,80%,45%,0.4),rgb(110,30,90,0.3),hsl(160,80%,45%,0.4));">
                                    ${table}
                                  </body>`;
  
  return pageWithRedBackground;
}
app.get('/Out', async function (req, res) {
    try {
      const fullname = req.query.persons;
      const mobilenumber = req.query.phnos;
      const originate = req.query.localitys.toLowerCase(); // Convert to lowercase
